    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try {
                ...
                List<String> pastasEmpresas = S3Service.listarEmpresasClient();
                System.out.println("[Java] " + pastasEmpresas.size() + " empresas encontradas.");

                for (String empresaPasta : pastasEmpresas) {
                    System.out.println("Processando ambiente da empresa: " + empresaPasta.toUpperCase());

                    String caminhoIncidentes = String.format("client/%s/dashboard_incidentes.json", empresaPasta);
                    List<IncidenteTelaDTO> incidentesNovos = S3Service.buscarIncidentesDoS3(caminhoIncidentes);
